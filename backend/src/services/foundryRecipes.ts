import { getFoundryResponsesConfig } from "../config/env";
...
export type FoundryRecipesServiceErrorKind =
  | "timeout"
  | "network"
  | "http"
  | "invalid_response";

export class FoundryRecipesServiceError extends Error {
  constructor(
    public readonly kind: FoundryRecipesServiceErrorKind,
    message: string,
  ) {
    super(message);
...
const ensureDetailedSteps = (steps: string[]): string[] => {
...
const toRecipeId = (name: string): string => {
...
const parseOutputText = (payload: ResponsesApiPayload): string => {
  ...
  if (!text) {
    throw new FoundryRecipesServiceError(
      "invalid_response",
      "Model output text is missing.",
    );
  }
...
const normalizeRecipes = (raw: unknown): RecipeRecommendation[] => {
  ...
  const candidates = (raw as { recipes?: unknown }).recipes;
  if (!Array.isArray(candidates) || candidates.length === 0) {
    throw new FoundryRecipesServiceError(
      "invalid_response",
      "Model response does not contain recipes.",
    );
  }

  const recipes = candidates
    .map((candidate): RecipeRecommendation | null => {
      const item = candidate as RecipeRecommendationCandidate;
      if (
  ...
  if (recipes.length === 0) {
    throw new FoundryRecipesServiceError(
      "invalid_response",
      "Model response recipes have an invalid schema.",
    );
  }
...
const buildPrompt = (ingredients: string[]): string => `
...
export const generateRecipeRecommendations = async (
  ingredients: string[],
): Promise<RecipeRecommendation[]> => {
  const config = getFoundryResponsesConfig();
  ...
  try {
    ...
    if (isAbortError(error)) {
      throw new FoundryRecipesServiceError(
        "timeout",
        `Foundry request timed out after ${REQUEST_TIMEOUT_MS}ms`,
      );
    }
  ...
  if (!response.ok) {
    ...
    throw new FoundryRecipesServiceError(
      "http",
      `Foundry request failed (${response.status}): ${errorText || response.statusText}`,
    );
  }
  ...
  try {
    ...
    throw new FoundryRecipesServiceError(
      "invalid_response",
      `Foundry response parsing failed: ${reason}`,
    );
  }
  ...
  try {
    ...
    throw new FoundryRecipesServiceError(
      "invalid_response",
      `Model output is not valid JSON: ${reason}`,
    );
  }
